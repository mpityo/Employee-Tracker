...
function filterEmployeeBy (search) {
    let sql = `SELECT employee.id, employee.first_name, employee.last_name,
                role.title AS role,
                role.salary AS salary,
                departments.name AS department,
...
function updateEmployee (empId, propsToUpdate) {
    ...
    for (var i = 0; i < propsToUpdate.length; i++) {
        sql += `${propsToUpdate[i].name} = ?`;
        // if this is NOT the last entry to change, add in , for next
        if (i+1 != propsToUpdate.length) {
            sql += `, `
        // if is last entry, add in emp id
        } else {
            sql += ` WHERE id = ?`;
...
module.exports = { sortEmployeeBy, addEmployee, deleteEmployee, updateEmployee, filterEmployeeBy };